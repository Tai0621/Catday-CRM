...
export default async function EditRoomPage({
  ...
  async function save(data: FormData) {
    ...
    if (!name) redirect(`/rooms/${id}?err=name`)
    try {
      ...
      redirect(`/rooms/${id}?err=name-taken`) // unique name clash
    }
    redirect('/rooms')
  }
  ...
  async function remove() {
    ...
    if (count > 0) redirect(`/rooms/${id}?err=has-bookings`)
    ...
    <div className="max-w-md mx-auto space-y-5">
      <div>
        <div className="flex items-center gap-2 mb-1">
          <Link href="/rooms" className="text-xs cd-muted hover:underline">Room Tracker</Link>
      ...
      <form action={save} className="cd-card p-5 space-y-4">
        ...
        <div className="flex gap-3 pt-1">
          ...
          <Link href="/rooms" className="cd-btn-sec text-sm">Cancel</Link>
        </div>
      </form>
